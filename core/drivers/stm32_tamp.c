...
#define _TAMP_STM32MP21_OR_IN1RMP_PC4		U(0)
...
static struct stm32_tamp_conf int_tamp_mp21[] = {
	{ .id = INT_TAMP1 }, { .id = INT_TAMP2 }, { .id = INT_TAMP3 },
	{ .id = INT_TAMP4 }, { .id = INT_TAMP5 }, { .id = INT_TAMP6 },
	{ .id = INT_TAMP7 }, { .id = INT_TAMP8 }, { .id = INT_TAMP9 },
...
static struct stm32_tamp_conf ext_tamp_mp21[] = {
	{ .id = EXT_TAMP1 }, { .id = EXT_TAMP2 }, { .id = EXT_TAMP3 },
	{ .id = EXT_TAMP4 }, { .id = EXT_TAMP5 }, { .id = EXT_TAMP6 },
	{ .id = EXT_TAMP7 },
};
					   ...
					   uint32_t (*cb)(int id))
{
	...
	if (res) {
		EMSG("Could not activate internal tamper:%u", tamp_int->id + 1);
		return res;
	}
...
static enum itr_return stm32_tamp_it_handler(struct itr_handler *h __unused)
{
	...
	while (int_it && i < stm32_tamp.pdata.compat->int_tamp_size) {
		uint32_t id = stm32_tamp.pdata.compat->int_tamp[i].id;
		...
		if (int_it & _TAMP_SR_ITAMP(id)) {
			...
			if (stm32_tamp.pdata.compat->int_tamp[i].func)
				ret =
				stm32_tamp.pdata.compat->int_tamp[i].func(id);
	...
	while (ext_it && i < stm32_tamp.pdata.compat->ext_tamp_size) {
		uint32_t id = stm32_tamp.pdata.compat->ext_tamp[i].id;
		...
		if (ext_it & _TAMP_SR_ETAMP(id)) {
			...
			if (stm32_tamp.pdata.compat->ext_tamp[i].func)
				ret =
				stm32_tamp.pdata.compat->ext_tamp[i].func(id);
