...
static void stm32_tamp_nvram_put_cell(struct nvmem_cell *cell)
{
	free(cell);
}
...
static const struct nvmem_ops stm32_tamp_nvram_nvmem_ops = {
	...
	.put_cell = stm32_tamp_nvram_put_cell,
};

static TEE_Result stm32_tamp_nvram_dt_get(struct dt_pargs *a, void *data,
					 ...
					 const void *compat_data)
{
	...
	pbase = fdt_reg_base_address(fdt, parent_node);
	if (pbase == DT_INFO_INVALID_REG) {
		ret = TEE_ERROR_BAD_PARAMETERS;
		goto free_nvram;
	}

	tamp_size = fdt_reg_size(fdt, parent_node);
